...
export const metadata = buildMetadata({
  ...
  description: "Answers to common questions about booking with Air1 Tickets: payments, changes and refunds, baggage rules, check-in, travel documents and how to reach our 24/7 US-based team.",
  path: "/help",
});
    ...
    <>
      ...
      <div className="container-page grid gap-10 py-12 lg:grid-cols-[1fr_18rem]">
        ...
        <aside className="space-y-4 lg:sticky lg:top-[calc(var(--header-height)+1rem)] lg:self-start">
          <nav aria-label="Help topics" className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">On this page</p>
            <ul className="mt-3 space-y-1.5 text-sm">
              {FAQ_GROUPS.map((g) => (
                <li key={g.id}>
                  <a href={`#${g.id}`} className="text-navy-900 hover:text-ocean-700 hover:underline">
                    {g.title}
                  </a>
                </li>
              ))}
          ...
          <div className="rounded-2xl bg-navy-900 p-5 text-white">
            ...
            <p className="mt-1 text-sm text-white/75">24 hours a day, 7 days a week, from the US.</p>
            <a href={`tel:${site.supportPhone.replace(/[^\d+]/g, "")}`} className="mt-3 flex items-center gap-2 font-semibold">
