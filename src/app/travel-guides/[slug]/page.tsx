  ...
  for (const s of slugs) {
    ...
    if (m) {
      ...
      if (o && dd) {
        ...
        if (oa && da)
          routes.push({
            href: routePath(oa, da),
            label: `Cheap flights from ${o.city} to ${dd.city}`,
          });
      }
    }
  }
...
export default async function ArticlePage({ params }: PageProps) {
  ...
  const more = ARTICLES.filter((x) => x.slug !== a.slug)
    .sort(
      (x, y) =>
        Number(y.category === a.category) - Number(x.category === a.category),
    )
    .slice(0, 3);
    ...
    <>
      <JsonLd
        data={[
          ...
          ...(a.faqs?.length ? [faqPageJsonLd(a.faqs)] : []),
        ]}
      />
        ...
        <div className="container-page pt-6 sm:pt-8">
          ...
          <div className="mt-6 grid gap-8 pb-8 lg:grid-cols-[1.2fr_1fr] lg:items-center">
            <div>
              ...
              <h1 className="mt-3 text-3xl sm:text-4xl lg:text-[2.6rem] lg:leading-[1.1]">
                {a.title}
              </h1>
      ...
      <div className="container-page grid gap-10 py-12 lg:grid-cols-[16rem_1fr]">
        <aside className="space-y-4 lg:sticky lg:top-[calc(var(--header-height)+1rem)] lg:self-start">
          ...
          <div className="rounded-2xl bg-navy-900 p-5 text-white">
            ...
            <p className="mt-1 text-sm text-white/75">
              Compare fares from 500+ airlines with the true total price shown
              up front.
            </p>
        ...
        <article className="max-w-3xl">
          ...
          <aside
            className="mt-8 rounded-2xl border border-sunrise-200 bg-sunrise-50 p-5"
            aria-labelledby="takeaways-heading"
          >
            <p
              id="takeaways-heading"
              className="flex items-center gap-2 font-display text-base font-bold text-navy-900"
            >
              <Lightbulb className="h-5 w-5 text-sunrise-600" aria-hidden /> Key
              takeaways
            </p>
            <ul className="mt-3 space-y-2 text-sm text-slate-700">
              {a.takeaways.map((t) => (
                <li key={t.slice(0, 40)} className="flex gap-2">
                  <span
                    className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-sunrise-500"
                    aria-hidden
                  />
                  <span>{t}</span>
                </li>
              ...
              <ul className="mt-4 grid gap-3 sm:grid-cols-2">
                {related.routes.map((r) => (
                  <li key={r.href}>
                    <Link
                      href={r.href}
                      className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-navy-900 transition hover:border-ocean-300"
                    >
                      {r.label}{" "}
                      <ArrowRight
          ...
          <p className="mt-12 rounded-2xl border border-slate-200 bg-slate-50 p-5 text-sm text-slate-600">
            Written by {a.author.name}. First published{" "}
            <time dateTime={a.publishedAt}>
              {formatDateLong(a.publishedAt)}
            </time>
            ; fees, rules and prices verified as of the update date above.
            Airline policies change: always confirm the fare rules shown at
            checkout for your specific ticket.
          </p>
        </article>
      </div>
      ...
      <section className="container-page py-12">
        <SectionHeading
          title="More guides"
          link={{ href: "/travel-guides", label: "All travel guides" }}
        />
        <ul className="mt-6 grid gap-4 sm:grid-cols-3">
          {more.map((m) => (
            <li key={m.slug}>
              <Link
                href={articlePath(m.slug)}
              ...
              >
                <div className="aspect-[16/9]">
                  <DestinationArt
                    ...
                    gradient={m.gradient}
                    seed={`article-${m.slug}`}
                    image={articlePhoto(m.slug) ?? undefined}
                    alt={m.title}
                  />
                </div>
                <div className="p-4">
                  ...
                  <p className="mt-1 font-semibold text-navy-900 group-hover:text-ocean-700">
                    {m.title}
                  </p>
